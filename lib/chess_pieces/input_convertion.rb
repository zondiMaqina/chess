...
class PieceSelection < PlayersPieces
  ...
  def find_valid_moves(opponent_pieces, all_directions, position, chess_board, player_pieces)
    row = position[0]
    col = position[1]
    case convert_position(chess_board[row][col])
      when :pawn then pawn_valid_moves(opponent_pieces, all_directions, row, col, chess_board, player_pieces)
      when :rook then rook_valid_moves(opponent_pieces, player_pieces, row, col, chess_board)
